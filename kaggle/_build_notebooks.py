...
nb02 = [
    ...
    code(
        "# ScienceQA (test split, image-bearing rows only). 200 tasks ~= 5 minutes.",
        "OUT = '/kaggle/working/gvla-data/scienceqa'",
        "!python scripts/prepare_scienceqa.py --split test --out-dir {OUT} --limit 200",
        "!ls {OUT} && wc -l {OUT}/test.jsonl",
    ),
    code(
        "# Mind2Web (Multimodal variant; the plain `osunlp/Mind2Web` is DOM-only).",
        "# The script probes available splits and falls back gracefully if",
        "# `test_task` isn't exposed in streaming mode.",
        "# 100 tasks ~= 8-15 min due to screenshot decode.",
        "OUT = '/kaggle/working/gvla-data/mind2web'",
        "!python scripts/prepare_mind2web.py \\",
        "    --dataset-id osunlp/Multimodal-Mind2Web \\",
        "    --split test_task \\",
        "    --out-dir {OUT} --limit 100",
        "!ls {OUT} && ls {OUT}/*.jsonl | head && wc -l {OUT}/*.jsonl",
    ),
    code(
